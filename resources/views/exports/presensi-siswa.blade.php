...
<body>
    @php
        ...
        $rekap = [
            ...
            'Bolos' => $presensis->where('statusPulang', 'Bolos')->count(),
        ];

        $bulanText = Carbon::create()->month((int) $bulan)->translatedFormat('F') . ' ' . $tahun;
        $urlVerifikasi = route('laporan.siswa.verifikasi', [
            'id' => $siswa->id,
            'bulan' => $bulan,
            'tahun' => $tahun,
        ]);
    ...
    <div class="logo-container">
        @php
            $logoPath = storage_path('app/public/' . $siswa->jabatan->instansi->logoInstansi);
        @endphp

        @if (!empty($siswa->jabatan->instansi->logoInstansi) && file_exists($logoPath))
    ...
    <div class="footer">
        <div class="footer-text">
            ...
            <p>Oleh Sistem Presensi {{ $siswa->jabatan->instansi->nama ?? 'Instansi' }}</p>
        </div>
        <div class="qr-code">
