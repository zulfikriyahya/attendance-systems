...
use App\Jobs\PenetapanHadirSiswaJob;
use App\Jobs\PenetapanLiburSiswaJob;
use App\Jobs\PenetapanDispenJob;
...
class PresensiSiswaResource extends Resource
{
    ...
    protected static ?string $slug = 'presensi-siswa';
        ...
        return $table
            ->headerActions([
                ...
                Action::make('set-hadir')
                    ...
                    ->action(function (array $data) {
                        ...
                        PenetapanHadirSiswaJob::dispatch($data);

                        // Notifikasi bahwa job telah dijadwalkan
                        Notification::make()
                            ->title('Penetapan Hadir Dijadwalkan')
                            ->body('Proses penetapan hadir telah dijadwalkan dan akan dijalankan di background.')
                            ->info()
                            ->send();
                    })
                ...
                Action::make('set-libur')
                    ...
                    ->action(function (array $data) {
                        // Dispatch job ke queue
                        PenetapanLiburSiswaJob::dispatch($data);

                        // Notifikasi bahwa job telah dijadwalkan
                        Notification::make()
                            ->title('Penetapan Libur Dijadwalkan')
                            ->body('Proses penetapan libur telah dijadwalkan dan akan dijalankan di background.')
                            ->info()
                            ->send();
                    })
                    ->visible(fn() => Auth::user()->hasRole('super_admin')),
                ...
                Action::make('set-dispen')
                    ...
                    ->action(function (array $data) {
                        // Dispatch job ke queue
                        PenetapanDispenJob::dispatch($data);

                        // Notifikasi bahwa job telah dijadwalkan
                        Notification::make()
                            ->title('Penetapan Dispen Dijadwalkan')
                            ->body('Proses penetapan dispen telah dijadwalkan dan akan dijalankan di background.')
                            ->info()
                            ->send();
                    })
                    ->visible(fn() => Auth::user()->hasRole('super_admin')),
            ...
            ->filters([
                SelectFilter::make('statusApproval')
                    ...
                    ->visible(Auth::user()->hasAnyRole(['super_admin', 'wali_kelas'])),
