...
class AllStatsOverview extends BaseWidget
{
    ...
    {
        return 'Statistik Presensi Pegawai';
    }
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make(
                'Status Sakit, Izin, Dinas Luar',
                PresensiPegawai::query()
                    ->whereIn('statusPresensi', ['Sakit', 'Izin', 'Dinas Luar'])
                    ->when($pegawaiId, fn($query) => $query->where('pegawai_id', $pegawaiId))
                    ->count() . ' Hari'
            )
                ...
                ->extraAttributes([
                    'class' => 'cursor-pointer',
                    'wire:click' => "\$dispatch('setStatusFilter', { filter: ['Sakit','Izin','Dinas Luar'] })",
                ]),
